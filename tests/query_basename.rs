//! Everything-class `basename:` alias of `name:` / `n:` / `filename:` (basename only).
...
#[test]
fn basename_is_an_alias_of_name() {
    let name = parse_query("name:alpha", now());
    for input in ["basename:alpha", "BASENAME:alpha", "Basename:alpha", "filename:alpha"] {
...
#[test]
fn basename_attached_rest_matches_name() {
    let name = parse_query("name:work", now());
    for input in ["basename:work", "BASENAME:work", "Basename:work"] {
        let q = parse_query(input, now());
        assert!(q.name_only, "{input}");
        assert!(!q.files_only, "{input}");
    ...
    let q = parse_query("basename:work", now());
    match &q.must[0] {
...
#[test]
fn basename_bare_sets_name_only() {
    let name = parse_query("name:", now());
    for input in ["basename:", "BASENAME:", "filename:"] {
        let q = parse_query(input, now());
        assert_eq!(q.name_only, name.name_only, "{input}");
        assert!(q.name_only, "{input}");
...
#[test]
fn basename_is_a_name_filter_not_a_term() {
    let q = parse_query("report basename:", now());
    assert!(q.name_only);
    assert!(!q.files_only);
    ...
    let q = parse_query("report basename:alpha", now());
    assert!(q.name_only);
...
#[test]
fn basenames_is_not_a_name_filter() {
    let q = parse_query("basenames:", now());
    assert!(!q.name_only);
    assert!(!q.files_only);
    assert_eq!(q.must.len(), 1);
    match &q.must[0] {
        Atom::Term(p) => assert_eq!(p.raw, "basenames:"),
        other => panic!("expected Term, got {other:?}"),
    }

    let q = parse_query("basenames:work", now());
    assert!(!q.name_only);
    assert_eq!(q.must.len(), 1);
    match &q.must[0] {
        Atom::Term(p) => assert_eq!(p.raw, "basenames:work"),
        other => panic!("expected Term, got {other:?}"),
    }
}

#[test]
fn basename_matches_basename_only_like_name() {
    ...
    for input in ["n:alpha", "name:alpha", "filename:alpha", "basename:alpha", "BASENAME:alpha"] {
